  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const usernameResponse = await axios.get(`https://api.waning.cloud/users/${user._id}`);
        setUserName(usernameResponse.data.username);
        
        // Fazer a requisição do nome dos amigos utilizando o ID
        const friendsResponse = await axios.get(`https://api.waning.cloud/users/${user._id}/friends`);
        setFollowing(friendsResponse.data);

        // Fazer a requisição do número de sonhos e pesadelos postados
        const dreamResponse = await axios.get(`https://api.waning.cloud/posts/${user._id}/posts?category=dreams`);
        setDreamCount(dreamResponse.data.length);

        const nightmareResponse = await axios.get(`https://api.waning.cloud/posts/${user._id}/posts?category=nightmares`);
        setNightmareCount(nightmareResponse.data.length);
        
        // Fazer a requisição dos posts do usuário logado utilizando o ID
        const response = await axios.get(`https://api.waning.cloud/posts/${user._id}/posts`);
