  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const usernameResponse = await axios.get(`https://crescent-api.vercel.app/users/${user._id}`);
        ...
        const dreamResponse = await axios.get(`https://crescent-api.vercel.app/posts/${user._id}/posts?category=dreams`);
        setDreamCount(dreamResponse.data.length);

        const nightmareResponse = await axios.get(`https://crescent-api.vercel.app/posts/${user._id}/posts?category=nightmares`);
        setNightmareCount(nightmareResponse.data.length);
        
        // Fazer a requisição dos posts do usuário logado utilizando o ID
        const response = await axios.get(`https://crescent-api.vercel.app/posts/${user._id}/posts`);
