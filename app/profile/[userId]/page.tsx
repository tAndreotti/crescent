...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`https://crescent-api.vercel.app/posts/${userId}/posts`);
        ...
        try {
          const myIdResponse = await axios.get('https://crescent-api.vercel.app/auth/user', { withCredentials: true });
        ...
        const userResponse = await axios.get(`https://crescent-api.vercel.app/users/${userId}`);
        setUserName(userResponse.data.username);

        // Fazer a requisição do número de sonhos e pesadelos postados
        const dreamResponse = await axios.get(`https://crescent-api.vercel.app/posts/${userId}/posts?category=dreams`);
        setDreamCount(dreamResponse.data.length);

        const nightmareResponse = await axios.get(`https://crescent-api.vercel.app/posts/${userId}/posts?category=nightmares`);
        setNightmareCount(nightmareResponse.data.length);

        // Fazer a requisição do nome dos amigos utilizando o ID
        const friendsResponse = await axios.get(`https://crescent-api.vercel.app/users/${myId}/friends`);
  ...
  const handleFollow = async () => {
    try {
      if (isFriend) {
        // Remover o amigo
        await axios.patch(`https://crescent-api.vercel.app/users/${myId}/${friendId}`, { withCredentials: true });
        setIsFriend(false);
      } else {
        // Adicionar o amigo
        await axios.patch(`https://crescent-api.vercel.app/users/${myId}/${friendId}`, { withCredentials: true });
