...
const Dreams = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const response = await axios.get(`https://api.waning.cloud/posts/${postId}`);
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`https://api.waning.cloud/posts/${myId}/${postId}`, { withCredentials: true });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const postId = window.location.pathname.split('/').pop();
      const response = await axios.patch(`https://api.waning.cloud/posts/${myId}/${postId}`, {
        text,
